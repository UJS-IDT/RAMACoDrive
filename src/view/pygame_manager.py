...
from map_visual import CenterMap
...
class PyGameAgent(BaseAgent):
    ...
    def init_game_view(self, world, urban_waypoints, config):
        ...
        # self.screen = pygame.display.set_mode((self.width * 2, self.height), pygame.HWSURFACE | pygame.DOUBLEBUF)
        # self.font = pygame.font.Font(None, 20)
        # self.clock = pygame.time.Clock()
        # self.centerMap = CenterMap(world, world.get_map(), self.screen, config)
    ...
    def run_step(self, world):
        ...
        right_area = pygame.Rect(self.width // 2, 0, self.width // 2, self.height)

        # Create surfaces for each area
        left_surface = pygame.Surface((left_area.width, left_area.height))
        right_surface = pygame.Surface((right_area.width, right_area.height))

        # Draw the map and vehicles on each surface
        self.centerMap.tick(left_surface, scale_factor=1.0)
        self.centerMap.tick(right_surface, scale_factor=0.5)
        self.screen.blit(left_surface, left_area.topleft)
        self.screen.blit(right_surface, right_area.topleft)
