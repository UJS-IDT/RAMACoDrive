...
from post_processor.box import common_utils
...
def caluclate_tp_fp_range(det_boxes, det_score, gt_boxes, result_stat, iou_thresh,
...
def eval_final_results(result_stat, save_path, global_sort_detections, tag):
    dump_dict = {}
    ...
    output_file = tag + '_eval.yaml' if not global_sort_detections else tag + '_eval_global_sort.yaml'
    save_yaml(dump_dict, os.path.join(save_path, output_file))
