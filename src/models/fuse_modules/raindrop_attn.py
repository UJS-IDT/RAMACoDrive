...
# import ipdb
from models.sub_modules.torch_transformation_utils_cobevflow import warp_affine_simple
...
class EncoderLayer_temporal(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        ...
        x = self.sublayer[0](q, lambda q: self.self_attn(q, k, v, mask))
...
class raindrop_fuse(nn.Module):
    ...
    def forward(self, x, rm, record_len, pairwise_t_matrix, time_diffs, batch_time_intervals=None, communication_masks_tensor=None, backbone=None, heads=None, noise_pairwise_t_matrix=None):
        """
        Fusion forwarding.
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                if i != 0:
                    x = backbone.resnet.blocks[i](x) if with_resnet else backbone.blocks[i](x)
            ...
            for b in range(B):
                ...
                t_matrix = pairwise_t_matrix[b][:N, :, :, :].view(-1, 2, 3) #(Nxk, 2, 3)
                node_features = x[b]
                #node_features = node_features * communication_masks_tensor[b]
                neighbor_feature = warp_affine_simple(node_features,
                                                t_matrix,
                                                (H, W))
                record_frames = np.ones((N))*K
                ...
                if self.agg_mode == "RAIN":
                    ...
                    sensor_dist = -1# (B, H, W)
                    x_fuse.append(self.fuse_modules(neighbor_feature, sensor_dist, batch_time_intervals[b], communication_masks_tensor[b]))
