    ...
    def render(self, display, receiver):
        ...
        for cav in cavs:
            ...
            if self.mem_dic[agent_mem].buf:
                timestamp = np.ndarray((1), dtype=np.float64, buffer=self.mem_dic[agent_mem].buf)[0]
                ...
                if True:
                    ...
                    metadata = np.ndarray((3), dtype=np.float64, buffer=self.mem_dic[agent_mem].buf)
                    flattened_lidar_data_length = int(metadata[1])
                    flattened_pose_length = int(metadata[2])
                    combined_data = np.ndarray((3 + flattened_lidar_data_length + flattened_pose_length), dtype=np.float64, buffer=self.mem_dic[agent_mem].buf)
...
def game_loop(args):
    ...
    try:
        ...
        # receiver.init_subscriber("sensor_data", 7901)  # 假设发送方使用的端口是12345
