...
class World(object):
    ...
    def _split_actors(self):
        ...
        return (vehicles, traffic_lights, speed_limits, walkers, cavs)
    ...
    def render(self, display, receivers):
        ...
        vehicles, traffic_lights, speed_limits, walkers, cavs = self._split_actors()
        ...
        if self.ego_exist:
            ...
            if self.mem_dic[self.ego_exist].buf:
                timestamp = np.ndarray((1), dtype=np.float64, buffer=self.mem_dic[self.ego_exist].buf)[0]
                ...
                if True:
                    ...
                    metadata = np.ndarray((3), dtype=np.float64, buffer=self.mem_dic[self.ego_exist].buf)
                    flattened_lidar_data_length = int(metadata[1])
                    flattened_pose_length = int(metadata[2])
                    combined_data = np.ndarray((3 + flattened_lidar_data_length + flattened_pose_length), dtype=np.float64, buffer=self.mem_dic[self.ego_exist].buf)
                    flattened_lidar_data = combined_data[3:3 + flattened_lidar_data_length]
                    latest_pose = combined_data[3 + flattened_lidar_data_length:3 + flattened_lidar_data_length + flattened_pose_length]
                    all_lidar_data = flattened_lidar_data.reshape((-1, 4))

                    if len(all_lidar_data) > 0:
                        lidar_to_world = transform_lidar_to_world(all_lidar_data, latest_pose)
                        points = self.map_image.world_to_pixel_fast(lidar_to_world)
                        points += translation
...
def game_loop(args):
    ...
    try:
        ...
        for agent_info in config_lidar["agents"]:
            ...
            receiver.init_subscriber(agent_info["v2x_name"], agent_info["inter_port"], "192.168.31.248")
