...
COLOR_ALUMINIUM_5 = pygame.Color(0, 0, 0)
...
args_lidar = recieve_args()
config_lidar = cfg.merge(args_lidar)
...
class MapImage(object):
    ...
    def world_to_pixel_fast(self, location, offset=(0, 0)):
        """Converts the world coordinates to pixel coordinates"""
        x = location[:, 0]
        y = location[:, 1]

        pixel_x = self.scale * self._pixels_per_meter * (x - self._world_offset[0])
        pixel_y = self.scale * self._pixels_per_meter * (y - self._world_offset[1])
        pixel_coordinates = np.vstack((pixel_x, pixel_y)).T
...
class World(object):
    ...
    def __init__(self, name, args, timeout):
        ...
        self.received_data_dic = {}
        self.mem_dic = {}
        self.last_timestamp = {}
        for cav_agent in config_lidar["agents"]:
            cav_mem = shared_memory.SharedMemory(name=cav_agent["share_mem"])
            self.mem_dic[cav_agent["name"]] = cav_mem
            self.last_timestamp[cav_agent["name"]] = -1.
        for road_agent in config_lidar["road_agents"]:
            road_mem = shared_memory.SharedMemory(name=road_agent["share_mem"])
            self.mem_dic[road_agent["name"]] = road_mem
            self.last_timestamp[road_agent["name"]] = -1.
    ...
    def _split_actors(self):
        ...
        for actor_with_transform in self.actors_with_transforms:
            ...
            if 'vehicle.ford.crown' in actor.type_id:
                cavs.append(actor_with_transform)
            elif 'static.prop.streetsign' in actor.type_id:
                roads.append(actor_with_transform)
        ...
        return (vehicles, traffic_lights, speed_limits, walkers, cavs, roads)
    ...
    def _render_vehicles(self, surface, list_v, world_to_pixel):
        ...
        for v in list_v:
            ...
            if 'cav' in v[0].attributes['role_name']:
                color = COLOR_CHAMELEON_0
            # Compute bounding box points
    ...
    def _render_cavs(self, surface, list_cavs, world_to_pixel):
        """Renders the vehicles' bounding boxes"""
        for v in list_cavs:
            color = COLOR_SKY_BLUE_0
            for agent in config_lidar["agents"]:
                if v[0].attributes['role_name'] in agent["name"]:
                    agent_color = agent["point_color"]
                    color = pygame.Color(agent_color[0], agent_color[1], agent_color[2])
                    pass
                # color = COLOR_CHAMELEON_0
            # Compute bounding box points
            bb = v[0].bounding_box.extent
    ...
    def render_actors(self, surface, vehicles, traffic_lights, speed_limits, walkers, cavs):
        ...
        self._render_cavs(surface, cavs, self.map_image.world_to_pixel)
    ...
    def calculate_distance(self, lidar_pose, actor_location):
        """
        lidar_pose: list[float]
        actor_location: carla.Location
        """
        lidar_x, lidar_y, lidar_z = lidar_pose[:3]
        actor_x, actor_y, actor_z = actor_location.x, actor_location.y, actor_location.z
        distance = np.sqrt((lidar_x - actor_x) ** 2 + (lidar_y - actor_y) ** 2 + (lidar_z - actor_z) ** 2)
        return distance

    def render(self, display, receivers):
        ...
        vehicles, traffic_lights, speed_limits, walkers, cavs, roads = self._split_actors()
        ...
        # center_offset = (0, 0)
        # center_lidar_offset = (0, 0)
        translation_lidar_offset = (self._input.mouse_offset[0] * scale_factor + self.scale_offset[0],
                                    self._input.mouse_offset[1] * scale_factor + self.scale_offset[1])
        center_lidar_offset = (abs(display.get_width() - self.surface_size) / 2 * scale_factor, 0)
        translation = np.array([translation_lidar_offset[0] + center_lidar_offset[0],
                                translation_lidar_offset[1] + center_lidar_offset[1]])
        received_data = None
        self.ego_exist = None
        if ego_actor is not None:
            if len(cavs) > 0:
                for cav in cavs:
                    if ego_actor == cav[0].attributes['role_name']:
                        self.ego_exist = ego_actor
                        break
        ...
        if self.ego_exist:
            ...
            ego_only = None
            for cav in cavs:
                if self.ego_exist == cav[0].attributes['role_name']:
                    ego_only = cav
                else:
                    not_ego.append(cav)
            for road in roads:
                if self.ego_exist == road[0].attributes['role_name']:
                    ego_only = road
                else:
                    not_ego.append(road)
            for cav in not_ego:
                # if ego_actor is not None and (ego_actor == cav[0].attributes['role_name']):
                for agent in config_lidar["agents"]:
                    if cav[0].attributes['role_name'] in agent["name"]:
                        cav_receiver = receivers[agent["name"]]
                        received_data = cav_receiver.receive_latest_message(agent["v2x_name"])
                        if received_data is not None:
                            self.received_data_dic[agent["name"]] = received_data
                        cav_color = agent["point_color"]
                        if agent["name"] in self.received_data_dic and self.received_data_dic[agent["name"]] is not None:
                            lidar_data = self.received_data_dic[agent["name"]][-1]["lidar_data"]
                            lidar_pose = self.received_data_dic[agent["name"]][-1]["lidar_pose"]
                            translation_lidar_offset = (self._input.mouse_offset[0] * scale_factor + self.scale_offset[0],
                                                    self._input.mouse_offset[1] * scale_factor + self.scale_offset[1])
                            center_lidar_offset = (abs(display.get_width() - self.surface_size) / 2 * scale_factor, 0)
                            #lidar_data, lidar_pose = self.received_data_dic[agent["name"]]
                            comm_dis = self.calculate_distance(lidar_pose, ego_only[1].location)
                            if comm_dis > config_lidar["communication_distance"]:
                                pass
                            else:
                                self.transform_matrix = x_to_world(lidar_pose)
                                lidar_to_world = project_points_by_matrix_torch(lidar_data[:, :3], self.transform_matrix)
                                #lidar_to_world = transform_lidar_to_world(lidar_data, lidar_pose)
                                points = self.map_image.world_to_pixel_fast(lidar_to_world)
                                translation = np.array([translation_lidar_offset[0] + center_lidar_offset[0],
                                          translation_lidar_offset[1] + center_lidar_offset[1]])
                                points += translation

                                for point in points:
                                    pygame.draw.circle(display, (cav_color[0], cav_color[1], cav_color[2]), point.astype(int), 1)
                                break
                for road_agent in config_lidar["road_agents"]:
                    if cav[0].attributes['role_name'] in road_agent["name"]:
                        cav_receiver = receivers[road_agent["name"]]
                        received_data = cav_receiver.receive_latest_message(road_agent["v2x_name"])
                        if received_data is not None:
                            self.received_data_dic[road_agent["name"]] = received_data
                        cav_color = road_agent["point_color"]
                        if road_agent["name"] in self.received_data_dic and self.received_data_dic[road_agent["name"]] is not None:
                            lidar_data = self.received_data_dic[road_agent["name"]][-1]["lidar_data"]
                            lidar_pose = self.received_data_dic[road_agent["name"]][-1]["lidar_pose"]
                            translation_lidar_offset = (self._input.mouse_offset[0] * scale_factor + self.scale_offset[0],
                                                    self._input.mouse_offset[1] * scale_factor + self.scale_offset[1])
                            center_lidar_offset = (abs(display.get_width() - self.surface_size) / 2 * scale_factor, 0)
                            #lidar_data, lidar_pose = self.received_data_dic[agent["name"]]
                            ...
                            if comm_dis > config_lidar["communication_distance"]:
                                ...
                                for point in points:
                                    pygame.draw.circle(display, (cav_color[0], cav_color[1], cav_color[2]), point.astype(int), 1)
                                break
            if self.mem_dic[self.ego_exist].buf:
                s = bytes(self.mem_dic[self.ego_exist].buf[:20])
                index = s.find(b"~")

                # if timestamp != self.last_timestamp[agent_mem]:
                if index != -1:
                    head = s[0:index]
                    contentlength = int(head)
                    content = bytes(self.mem_dic[self.ego_exist].buf[index + 1:index + 1 + contentlength])
                    try:
                        data = pickle.loads(content)
                        timestamp = data["timestamp"]
                        self.last_timestamp[self.ego_exist] = timestamp
                        mem_color = [255, 255, 255]
                        for agent in config_lidar["agents"]:
                            if self.ego_exist == agent["name"]:
                                mem_color = agent["point_color"]
                        latest_pose = data["lidar_pose"]
                        all_lidar_data = data["lidar_data"]
                        translation_lidar_offset = (self._input.mouse_offset[0] * scale_factor + self.scale_offset[0],
                                                    self._input.mouse_offset[1] * scale_factor + self.scale_offset[1])
                        center_lidar_offset = (abs(display.get_width() - self.surface_size) / 2 * scale_factor, 0)
                        if len(all_lidar_data) > 0:
                            self.transform_matrix = x_to_world(latest_pose)
                            lidar_to_world = project_points_by_matrix_torch(all_lidar_data[:, :3], self.transform_matrix)
                            points = self.map_image.world_to_pixel_fast(lidar_to_world)
                            translation = np.array([translation_lidar_offset[0] + center_lidar_offset[0],
                                          translation_lidar_offset[1] + center_lidar_offset[1]])
                            points += translation
                            ...
                            for point in points:
                                pygame.draw.circle(display, (mem_color[0], mem_color[1], mem_color[2]), point.astype(int),1)
            ...
            for road in roads:
                # if ego_actor is not None and (ego_actor == cav[0].attributes['role_name']):
                for road_agent in config_lidar["road_agents"]:
                    if road[0].attributes['role_name'] in road_agent["name"]:
                        cav_receiver = receivers[road_agent["name"]]
                        received_data = cav_receiver.receive_latest_message(road_agent["v2x_name"])
                        if received_data is not None:
                            self.received_data_dic[road_agent["name"]] = received_data
...
def game_loop(args):
    ...
    try:
        ...
        receivers = {}
        for agent_info in config_lidar["agents"]:
            receiver = CommuniAgent("receiver" + agent_info["name"])
            receiver.init_subscriber(agent_info["v2x_name"], agent_info["inter_port"], config_lidar["comm_trans_addr"])
            receivers[agent_info['name']] = receiver
        for agent_info in config_lidar["road_agents"]:
            receiver = CommuniAgent("receiver" + agent_info["name"])
            receiver.init_subscriber(agent_info["v2x_name"], agent_info["inter_port"], config_lidar["comm_trans_addr"])
            receivers[agent_info['name']] = receiver
        # receiver = CommuniAgent("receiver")
        # receiver.init_subscriber("sensor_data", 7901)
        ...
        while True:
            ...
            # Render all modules
            # display.fill(COLOR_BLACK)
            world.render(display, receivers)
            hud.render(display)
        ...
        if world is not None:
            for key, value in world.mem_dic.items():
                value.close()
                resource_tracker.unregister(value._name, "shared_memory")
...
def lidar_to_bev(lidar, min_x=-100, max_x=100, min_y=-100, max_y=100, pixels_per_meter=4,
                     hist_max_per_pixel=2):
    xbins = np.linspace(min_x, max_x + 1,
                (max_x - min_x) * pixels_per_meter + 1,)
    ybins = np.linspace(min_y, max_y + 1,
...
def transform_lidar_to_world(lidar_points, lidar_transform):
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description='V2X Lidar Data Visualizer')
