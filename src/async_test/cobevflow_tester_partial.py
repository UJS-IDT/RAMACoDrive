...
class Tester(multiprocessing.Process):
    def __init__(self, ego_config, all_config, receive_mutex, cof_seg, eval_all_frame, eval_remain, model, init_epoch, saved_path):
        ...
        self.past_k_time_diffs_dict = {}
        self.this_time_diffs_dict = {}
        self.num_sweep_frames = self.config["hypes_yaml"]['model']['args']['num_sweep_frames']
                ...
                try:
                    ...
                    if self.ego_flag:
                            ...
                            for cav_id, cav_data in self.cav_data_dict.items():
                                ...
                                if len(single_cav_processed) == 0:
                                    ...
                                    if delayed_time_value > 45.0 or delayed_time_value < -1.0:
                                        continue
                                    processed_features.append(single_cav_processed["processed_features"])
                                    object_id_stack.extend([item for item in single_cav_processed["object_ids"] if item not in object_id_stack])
                                    if cav_id not in self.this_time_diffs_dict:
                                        self.this_time_diffs_dict[cav_id] = cav_data[scene_id]["timestamp"]
                                        self.past_k_time_diffs_dict[cav_id] = []
                                        past_k_time_diffs = 0.0
                                        self.past_k_time_diffs_dict[cav_id].append(past_k_time_diffs)
                                    else:
                                        this_time = cav_data[scene_id]["timestamp"]
                                        past_k_time_diffs = abs(this_time - self.this_time_diffs_dict[cav_id])
                                        self.past_k_time_diffs_dict[cav_id].append(past_k_time_diffs)
                                        self.this_time_diffs_dict[cav_id] = this_time
                                    transform_matrix_list.append(single_cav_processed['transform_matrix'])
                                    if self.num_sweep_frames == 1:
                                        past_k_time_diffs_stack.append(past_k_time_diffs)
                                    else:
                                        past_k_time_diffs = np.mean(self.past_k_time_diffs_dict[cav_id])
                                        past_k_time_diffs_stack.append(past_k_time_diffs)
                                    time_delay.append(delayed_time_value)
                                    if self.config["show_sequence"]:
                                        projected_lidar_stack.append(single_cav_processed['projected_lidar'])
                        ...
                        with torch.no_grad():
                            ...
                            self.ouput_dict = self.model(self.input_data)
    ...
    def collate_batch_train(self, batch):
        ...
        for i in range(len(batch)):
            ...
            past_k_time_diff.append(input_dict['past_k_time_diffs'])
            if self.config["show_sequence"]:
