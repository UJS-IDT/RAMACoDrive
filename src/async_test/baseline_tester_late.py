            ...
            if self.test_event.is_set():
                ...
                try:
                    ...
                    if self.ego_flag:
                            ...
                            for cav_id, cav_data in self.cav_data_dict.items():
                                ...
                                if len(single_cav_processed) == 0:
                                    ...
                                    if scene_id == 0 and cav_id != self.ego_config["name"]:
                                        self.late_result.append(cav_data[0]["late_result"])
                        ...
                        if len(self.late_result) > 0:
                            psm_result = []
                            rm_result = []
                            scores = []
                            mask = []
                            mask_reg = []
                            for dic in self.late_result:
                                psm_result.append(dic["psm_selected"])
                                rm_result.append(dic["rm_selected"])
                                scores.append(dic["scores"])
                                mask.append(dic["mask_indices"])
                                mask_reg.append(dic["mask_reg_indices"])
                            self.collate_batch.update({"psm_result_v2x": psm_result,
                                                       "rm_result_v2x": rm_result,
                                                       "scores_v2x": scores,
                                                       "mask_v2x": mask,
                                                       "mask_reg_v2x": mask_reg})
                        with torch.no_grad():
                            ...
                            if "psm_result_v2x" in self.input_data_cpu:
                                del self.input_data_cpu["psm_result_v2x"]
                            if "rm_result_v2x" in self.input_data_cpu:
                                del self.input_data_cpu["rm_result_v2x"]
                            if "scores_v2x" in self.input_data_cpu:
                                del self.input_data_cpu["scores_v2x"]
                            if "mask_v2x" in self.input_data_cpu:
                                del self.input_data_cpu["mask_v2x"]
                            if "mask_reg_v2x" in self.input_data_cpu:
                                del self.input_data_cpu["mask_reg_v2x"]
