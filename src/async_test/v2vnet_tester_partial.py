...
class Tester(multiprocessing.Process):
            ...
            if self.test_event.is_set():
                ...
                if self.eval_ready_flag:
                    ...
                    for key, value_cpu in tqdm(self.eval_valid_frame.items()):
                        ...
                        #relative_obj_id = self.get_relative_obj(range_obj_id, relative_obj_id)
                        ...
                        if self.config["show_sequence"]:
                            ...
                            vis_save_path = os.path.join(vis_save_path, '%05d.png' % vis_idx)
    ...
    def get_projected_lidar(self, lidar_raw_data, ego_pose, cav_pose):
        lidar_np = lidar_raw_data
        lidar_np = shuffle_points(lidar_np)
        lidar_np = mask_ego_points(lidar_np)
        transform_matrix = x1_to_x2(cav_pose, ego_pose)
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        if processed_flag:
            if not self.config["hypes_yaml"]["proj_first"]:
                projected_lidar = self.get_projected_lidar(selected_cav_base["lidar_data"], ego_pose, selected_cav_base["lidar_pose"])
