...
import sys
from util import spawn_vehicle, connect_to_server, time_const, thread_process_vehicles, get_ego_vehicle, get_speed, log_time_cost, get_vehicle_info, x_to_world, project_points_by_matrix_torch, x1_to_x2, to_device, create_model
...
class V2X_communicator(multiprocessing.Process):
    def __init__(self, all_config, config, receive_mutex, send_mutex, model, stop_process, record_cof=None):
        ...
        self.max_cav_num = self.all_config["hypes_yaml"]['train_params']['max_cav']
        self.model = model
        #self.model.eval()
                    ...
                    if self.mem_addr is not None and self.mem_addr.buf is not None:
                        ...
                        if index != -1:
                            ...
                            if data["timestamp"] != self.last_timestamp:
                                ...
                                project_lidar, cav_processed_feature, processed_flag = self.get_processed_lidar(temp_dict["lidar_data"], ego_pose, temp_dict["lidar_pose"])
                                if processed_flag is False:
                                    continue
                                temp_dict["processed_lidar"] = cav_processed_feature
                                temp_dict["project_lidar"] = project_lidar
                                if not self.config["ego_flag"]:
                                    del temp_dict["lidar_data"]
                                    del temp_dict["project_lidar"]
                    ...
                    if len(self.data_buffer) >= self.scene_size:
                        if not self.config["ego_flag"]:
                            ...
                            for i in range(len(self.data_buffer)):
                                input_dict = self.data_buffer[i]
                                processed_lidar_list.append(input_dict['processed_lidar'])
                                #del self.data_buffer[i]['processed_lidar']
                            ...
                            with torch.no_grad():
                                input_data = to_device(processed_lidar_torch_dict, self.device)
                                spatial_features_2d, valid_mask = self.model(input_data, False)
                                spatial_features_2d = to_device(spatial_features_2d, self.device_cpu)
                                valid_mask = to_device(valid_mask, self.device_cpu)
                            self.data_buffer[0]["spatial_features_2d"] = spatial_features_2d
                            self.data_buffer[0]["valid_mask"] = valid_mask
                            for i in range(len(self.data_buffer)):
                                del self.data_buffer[i]['processed_lidar']
                            self.send_lidar_feature(self.data_buffer)
                        else:
                            self.send_lidar_feature_ego(self.data_buffer)
                        self.data_buffer = []                                               
                except Exception as e:
                    #print(e)
                    traceback.print_exc()                    
    ...
    def send_lidar_feature_ego(self, lidar_feature_list):
        # Memory Share
        obj_data = pickle.dumps(lidar_feature_list)
        obj_len = len(obj_data)
        metadata = str(obj_len) + "~"
        ...
        # ZMQ Trans
        for lidar_feature in lidar_feature_list:
            if "lidar_data" in lidar_feature:
                del lidar_feature["lidar_data"]
        self.lidar_agent.lidar_agent.send_obj(lidar_feature_list)
    ...
    def get_processed_lidar(self, lidar_raw_data, ego_pose, cav_pose):
        ...
        if lidar_np.shape[0] == 0:
            ...
            return lidar_np, processed_lidar, True
