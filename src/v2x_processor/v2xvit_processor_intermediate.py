                                    ...
                                    for key, value in self.receivers.items():
                                        try:
                                            ...
                                            if received_data is not None:
                                                ...
                                                ego_pose = temp_dict["received_ego_pose"][key] #v2x-vit only supply one ego
                                            elif key in self.temp_ego_pose:
                                                temp_dict["received_ego_pose"][key] = self.temp_ego_pose[key]
                                                ego_pose = temp_dict["received_ego_pose"][key]
                    ...
                    if len(self.data_buffer) >= self.scene_size:
                        if not self.config["ego_flag"]:
                            ...
                            with torch.no_grad():
                                ...
                                spatial_features_2d = self.model(input_data, False)
