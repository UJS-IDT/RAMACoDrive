...
class PointPillarLoss(nn.Module):
    ...
    def logging(self, epoch, batch_id, writer, total_batches, pbar=None):
        """
        ...
        if pbar is None:
            print("[epoch %d][%d], || Loss: %.4f || Conf Loss: %.4f"
                " || Loc Loss: %.4f" % (
                    epoch, batch_id + 1,
                    total_loss.item(), conf_loss.item(), reg_loss.item()))
        else:
            pbar.set_description("[epoch %d][%d], || Loss: %.4f || Conf Loss: %.4f"
                  " || Loc Loss: %.4f" % (
                      epoch, batch_id + 1,
                      total_loss.item(), conf_loss.item(), reg_loss.item()))


        writer.add_scalar('Regression_loss', reg_loss.item(), total_batches)
        writer.add_scalar('Confidence_loss', conf_loss.item(), total_batches)


    def logging_backup(self, epoch, batch_id, batch_len, writer, pbar=None):
        """
        Print out  the loss function for current iteration.
