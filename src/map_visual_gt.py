...
class World(object):
    ...
    def select_hero_actor(self):
        ...
        for actor in self.world.get_actors():
            if 'lidar' in actor.type_id:
                print("actor===============", actor.parent)
                if "street" in actor.parent.type_id:
                    print("actor.transform", actor.get_transform())
        #     if 'lidar' in actor.type_id:
        #     if 'ford.crown' in actor.type_id:
        #         print("actor===============", actor)
        # lidar_actors = self.world.get_actors().filter('*lidar*')[0]
        # sensor_transform = lidar_actors.get_transform()
        # sensor_location = sensor_transform.location
        # print("actor===============", sensor_location)
    ...
    def render(self, display, receiver):
        ...
        for cav in cavs:
            agent_mem = cav[0].attributes["role_name"]
            if self.mem_dic[agent_mem].buf:
                s = bytes(self.mem_dic[agent_mem].buf[:20])
                index = s.find(b"~")
                if index != -1:
                    head = s[0:index]
                    contentlength = int(head)
                    content = bytes(self.mem_dic[agent_mem].buf[index + 1:index + 1 + contentlength])
                    try:
                        data = pickle.loads(content)
                        timestamp = data["timestamp"]
                        self.last_timestamp[agent_mem] = timestamp
                        mem_color = [255, 255, 255]
                        for agent in config_lidar["agents"]:
                            if agent_mem == agent["name"]:
        ...
        for road in roads:
            agent_mem = road[0].attributes["role_name"]
            if self.mem_dic[agent_mem].buf:
                s = bytes(self.mem_dic[agent_mem].buf[:20])
                index = s.find(b"~")
                if index != -1:
                    ...
                    try:
                        ...
                        self.last_timestamp[agent_mem] = timestamp
                        mem_color = [255, 255, 255]
                        for agent in config_lidar["road_agents"]:
                            if agent_mem == agent["name"]:
                                mem_color = agent["point_color"]
                        latest_pose = data["lidar_pose"]
                        all_lidar_data = data["lidar_data"]
...
def game_loop(args):
    ...
    try:
        ...
        for agent_info in config_lidar["agents"]:
            ...
            receiver.init_subscriber(agent_info["v2x_name"], agent_info["inter_port"])
...
def main():
    ...
    argparser = argparse.ArgumentParser(
        description='V2X Lidar Data Ground Truth Visualizer')
