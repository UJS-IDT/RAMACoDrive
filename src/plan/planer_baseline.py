...
from perception.perception_basline import PrivilegePerception
import logging
...
class FrenetPlanner():
    ...
    def update_waypoint_buffer(self):
        num_push = min(int(self.speed*3/5)+2,
        ...
        if curvature > 0.0001:
            self.target_speed = max(
                8, self.max_speed * math.exp(-6*curvature))
        else:
